classdef ef_covB_3_4_21_app < handle
% ef_covB_3_4_21_app Builds an app with the ability to detect binding events from noisy single
% molecule data and produce relevant figures including ensemble averages.
%   ef_covB_3_4_21_app creates a new app instance and displays a GUI for interacting with the
%   app.
%
%   ef_covB_3_4_21_app(txtInputFcn) accepts either a function handle or a character vector
...
%
%   ef_covB_3_4_21_app(txtInputFcn, txtOutputFcn) also accepts either a function handle or a
...
%
%   myApp = ef_covB_3_4_21_app( ___ ) returns the app instance in myApp. Use dot indexing to
...
% This program should be compatible with MATLAB releases R2017b through at least R2020a.
    ...
    properties (SetObservable, AbortSet)
        ...
        minSep = 100         % Events separated by less than minSep number of points are removed automatically.
        minDur = 100         % Events shorter than minDur number of points are removed automatically.
    ...
    methods
        ...
        function app = ef_covB_3_4_21_app(txtInputFcn, txtOutputFcn)
            % Constructor.
        ...
        function avg = getBeadsToAnalyze(app, other)
            % GETBEADSTOANALYZE Returns the average of the two beads' positions.
            %   avg = GETBEADSTOANALYZE(app) returns the averaged position of app's two beads,
            %   excluding data which was removed by the user. If the user has specified that only
            %   one bead should be analyzed, that bead's position will be returned instead.
            %   However, if other is true, the other bead's position will be returned instead.
    ...
    methods (Static)
        ...
        function cov = calculateCov(A, B, averagingWindow, smoothingWindow, tlbxAvailable)
            ...
            if nargin < 5
                tlbxAvailable = ef_covB_3_4_21_app.checkInstall('Signal Processing Toolbox');
            end
            
            % Calculate the covariance.
            A_filt = movmean(A, averagingWindow);
            B_filt = movmean(B, averagingWindow);
            AB_filt = movmean(B.*B, averagingWindow);
            cov = AB_filt - B_filt.*B_filt;
        ...
        function [T1, T2, Lr, idx] = changepoint(data, varargin)
            ...
            if numel(data) <= 1500
                ...
                [T1, T2, Lr] = ef_covB_3_4_21_app.findTwoChangepoints(data, onlyLr);
                idx = 1:numel(data);
            else
                ...
                subset = data(idx);
                [t1, t2, Lr] = ef_covB_3_4_21_app.findTwoChangepoints(subset, onlyLr); % [T1's estimate, T2's estimate, likelihood]
    ...
    methods (Access = private)
        ...
        function defaultClick(app, ~, ~)
            ...
            try
                ...
                if ~isempty(a)
                    ...
                    % If user clicked the covariance histogram axes, also add a submenu for
                    % manually setting the x limits.
                    if strcmp(a.UserData, 'covHistAxes')
                        uimenu(app.misc.cm,...
                            'Text', 'Change X Limits',...
                            'MenuSelectedFcn', @app.changeXLimitsCallback,...
                            'Enable', enableState);
                    end
        ...
        function changeXLimitsCallback(app, ~, ~)
            % Called when user clicks 'Change X Limits' button on the UIContextMenu of ax.
            % Opens a dialog box allowing them to set the X limits.
            ...
            try
                ...
                def = strtrim(cellstr(num2str(app.analyze.covHist.covHistPlot.BinLimits'))');
                answer = inputdlg({'Lower X limit:', 'Upper X limit:'}, '', 1, def);
                if isempty(answer)
                    uiwait(errordlg('Invalid input.', 'Error Dialog', 'modal'));
                    app.restore(store);
                    return
                end
                try
                ...
                app.analyze.covHist.covHistPlot.BinLimits = [low, high];
                ...
                app.unexpectedError(ME, sprintf('An unexpected error occurred:\n\n%s', ME.message));
                app.restore(store);
            end
        end
    ...
    methods (Access = private)
        ...
        function backup = storeBackupTab1(app)
            ...
            backup.analyze.covHist.covHistPlot.BinLimitsMode = app.analyze.covHist.covHistPlot.BinLimitsMode;
            backup.analyze.covHist.covHistPlot.BinLimits = app.analyze.covHist.covHistPlot.BinLimits;
        ...
        function restoreBackupTab1(app, backup, mode)
            ...
            if covHistQ
                ...
                app.analyze.covHist.covHistPlot.BinLimits = backup.analyze.covHist.covHistPlot.BinLimits;
                app.analyze.covHist.covHistPlot.BinLimitsMode = backup.analyze.covHist.covHistPlot.BinLimitsMode;
        ...
        function refinedEvents = refineEventsChangepoint(app, events)
            ...
            % Get the beads.
            averageBeads = app.getBeadsToAnalyze(true);
        ...
        function [avgbeg, avgend] = calcEnsemble(app)
            ...
            averageBeads = app.getBeadsToAnalyze(false);
            events = app.getActiveEvents;
