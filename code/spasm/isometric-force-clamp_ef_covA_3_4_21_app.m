classdef ef_covA_3_4_21_app < handle
% ef_covA_3_4_21_app Builds an app with the ability to detect binding events from noisy single
% molecule data and produce relevant figures including ensemble averages.
%   ef_covA_3_4_21_app creates a new app instance and displays a GUI for interacting with the
%   app.
%
%   ef_covA_3_4_21_app(txtInputFcn) accepts either a function handle or a character vector
...
%
%   ef_covA_3_4_21_app(txtInputFcn, txtOutputFcn) also accepts either a function handle or a
...
%
%   myApp = ef_covA_3_4_21_app( ___ ) returns the app instance in myApp. Use dot indexing to
...
% This program should be compatible with MATLAB releases R2017b through at least R2020a.
    ...
    methods
        ...
        function app = ef_covA_3_4_21_app(txtInputFcn, txtOutputFcn)
            % Constructor.
    ...
    methods (Static)
        ...
        function cov = calculateCov(A, B, averagingWindow, smoothingWindow, tlbxAvailable)
            ...
            if nargin < 5
                tlbxAvailable = ef_covA_3_4_21_app.checkInstall('Signal Processing Toolbox');
            end
            
            % Calculate the covariance.
            A_filt = movmean(A, averagingWindow);
            B_filt = movmean(B, averagingWindow);
            AB_filt = movmean(A.*A, averagingWindow);
            cov = AB_filt - A_filt.*A_filt;
        ...
        function [T1, T2, Lr, idx] = changepoint(data, varargin)
            ...
            if numel(data) <= 1500
                ...
                [T1, T2, Lr] = ef_covA_3_4_21_app.findTwoChangepoints(data, onlyLr);
                idx = 1:numel(data);
            else
                ...
                subset = data(idx);
                [t1, t2, Lr] = ef_covA_3_4_21_app.findTwoChangepoints(subset, onlyLr); % [T1's estimate, T2's estimate, likelihood]
